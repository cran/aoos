...
- You can use (and must endure) features of S4.
...
- `public`: To make something public
...
The return value of `defineClass` is the constructor function which is assigned to the name `Person`. All arguments to the constructor will be passed on to `init` if you defined it. In the above definition `init` will be private.
...
For every class you define with `defineClass` the S4 method `initialize` is set. Do not change it. You can also use the S4 constructor `new`, however you cannot pass anything to `init`.
